...
import (
	...
	apimock "github.com/videocoin/cloud-api/mock"
...
type APITestSuite struct {
	...
	streams    *apimock.MockStreamsServiceClient
}
...
func (suite *APITestSuite) SetupSuite() {
	...
	streams := apimock.NewMockStreamsServiceClient(mockCtrl)
	suite.streams = streams
...
func (suite *APITestSuite) TestUpload_FromFile() {
	streamResp := &pstreamsv1.StreamResponse{
		ID:     mock.StreamID,
		UserID: mock.UserID,
		Status: streamsv1.StreamStatusPrepared,
	}
	suite.streams.EXPECT().Get(gomock.Any(), gomock.Any()).Return(streamResp, nil).Times(1)
